...
2. Env vars (append is safe for new keys): `SSO_ENABLED=false`, `SCOUT_DRIVER=database`, `SESSION_DRIVER=database` (overrides the platform's `cookie` default, which breaks Passport OAuth - see foot-gun 14), and `APP_URL=<vanity url>` - Cloud only pre-sets APP_KEY, it does NOT set APP_URL.
...
14. **SESSION_DRIVER=cookie breaks Passport's OAuth approve page with a deterministic 403 (verified live 2026-08-16, devnotes).** Symptom: clicking Approve on `/oauth/authorize` returns Passport's "The provided auth token for the request is different from the session auth token." on every attempt, while normal login sessions work perfectly. Cause: the authorize GET puts a random `authToken` AND the whole `serialize()`d authRequest into the session (Passport AuthorizationController); with the cookie driver that payload must travel back as a Set-Cookie, blows the ~4KB browser cookie limit, and the browser drops it silently - the old (small) cookie keeps you logged in, the authToken never reaches the browser, and the approve POST fails Passport's one-shot `pull()` comparison. Diagnosis breadcrumb: access logs show the same IP/UA doing GET `/oauth/authorize` 200 then POST 403 seconds later, no double GET, login unaffected. Fix: `SESSION_DRIVER=database` (the sessions table ships in the stock `0001_01_01_000000_create_users_table` migration) and redeploy. `cookie` IS the Cloud platform default (confirmed 2026-08-16 in the dashboard: a read-only "defaults" area shows SESSION_DRIVER=cookie; overriding means duplicating the key with a new value in the editable env area). So EVERY Passport/MCP-OAuth app on Cloud hits this until the key is overridden to `database` - make the override part of standing setup, and expect the same failure from anything else that stuffs a large payload into the session.
