...
- **Brand design skills**: Check if the `uofg-design-system` skill (or similar brand skill) is available. If found, note that a brand system exists — this will inform Step 1b.

Note what you've learned and what remains unclear.

### Step 1b: Detect Brand Mode

If you detected a brand design skill (e.g. `uofg-design-system`) during Step 1, or the user mentions working within an institutional/corporate brand, ask the user which **design mode** applies to this project:

1. **Pure brand** — Strict brand compliance. Recommend the user uses the brand skill directly (e.g. `uofg-design-system`) instead of Impeccable. Stop the teach flow here.
2. **Brand-creative** — Brand foundations (colours, primary typeface, spacing scale, accessibility requirements) are locked in as constraints, but Impeccable's creative thinking applies to layout, motion, visual rhythm, interaction, and surface treatments. A secondary display font may be paired alongside the brand typeface. The brand palette is the starting point, not the ceiling — derived tints, surface colours, and richer treatments are encouraged so long as the core brand colours remain recognisable.
3. **Unconstrained** — No brand constraints. Full Impeccable creative freedom. Proceed normally.

If no brand skill was detected and the user doesn't mention one, skip this step and proceed as unconstrained.
...
Skip questions where the answer is already clear from the codebase exploration. In **brand-creative** mode, skip questions already answered by the brand skill (e.g. colours, accessibility level, primary typeface) and focus on the creative latitude: what feeling should the piece evoke beyond the brand baseline, where can it be playful, what's the audience that goes beyond the institution's usual audience.
...
### Design Mode
[unconstrained | brand-creative | pure brand]
[If brand-creative: name the brand skill and summarise what is locked vs what has creative latitude]
...
[If brand-creative: note the brand palette and typeface as the foundation, then describe the creative direction layered on top — secondary display font, motion, spatial rhythm, surface treatments]
