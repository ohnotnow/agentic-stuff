...
## Some local conventions to help you

- We have an `is_staff` flag on the user models.  We are a .ac.uk so that flag just indicates that the user is a member of staff (academic or admin).  Students have it set to false.  Before making any conclusions based on the flag check things like routes and middleware - some apps block students even logging in via the SSOController.
- Audits for `npm`.  We generally only audit npm locally as our primary use for it is to build our css rather than bundle javascript.  That isn't always the case for older apps.  You can generally tell which we consider old or new based on the composer.json having livewire+flux == new, things like bulma or bootstrap being in package.json indicate an older app.
- PHP static analysis - if phpstan/larastan are in the composer.json then we use them 'as and when' - we don't enforce them.  That is our established convention.  
...
- Check `ant` (you have a skill available for this) to see if the project as a foundation document or other ADR/notes to help you ground in the project and decisions already made.
- Check `ait` (a simple keyword `ait search <term>` should be enough) to see if there are any tasks or epics related to what you are auditing.
...
- The project's users are all full-time staff and students of a UK University.  The application is not available off-campus.  Please bare that in mind.  Senior researchers, full-time contracted administrators and students paying for their education are less likely to be hacking about in their browsers developer tools to 'hack' an app.
...
If the project is using `ait` or `ant` - check to see if there are concrete notes or ADR decisions that can be used to guide you.  If you find something which seems to contradict something surfaced in Phase 2 - mention it in your findings.
